def Churn_ForOpt(cost, cost_call, cost_sms, Competitor1_offer):
    # (Data cost, call rate, sms rate, Competitor 1)
    import mesa
    import pandas as pd
    ...
    data_set = CONFIG['Model']['syn_dataset']
    ...
    month= 'Jan' #input("Enter the Month in this format 'Mon'") # Month Selection
    ...
    resolution_efficiency = 0.7 # float(input("Enter average probability that CC executive is able to resovle query: ")) # CC Efficiency
    Churn_Threshold = 0.9 # float(input("Enter the threshold for churn probability: ")) # Churn Threshold
    #Competitor1_offer = float(input("Enter the Competitor 1 Discount: ")) # Competitor 1 Offer
    Competitor2_offer = 1000 # float(input("Enter the Competitor 2 Discount: ")) # Competitor 2 Offer
    Competitor_offer = max(Competitor1_offer,Competitor2_offer)
    Self_offer = 1000 # float(input("Enter the Self Discount: "))  # Self Offer
    ...
    df1["Individual_Account_Revenue"] = (1-df1["Churn_Binary"])*60*(1+(cost-0.05)/0.05/750)*(1+(cost_call-0.05)/0.05/250)*(1+(cost_sms-0.05)/0.05/300)
    df1["LTV"] = df3["ltv_values"] #LTV
    #df1["ChurnCount"] = ''
    #df1["ChurnCount"].iloc[0]= df1["Churn_Binary"].sum()
    Churn_total = df1["Churn_Binary"].sum()
    # df1["TotalRevenue"] = ''
    # df1["TotalRevenue"].iloc[0]= df1["Individual_Account_Revenue"].sum()
    Revenue_total = df1["Individual_Account_Revenue"].sum()
    ...
    #print(Churn_total)
    return (Churn_total, Revenue_total)
