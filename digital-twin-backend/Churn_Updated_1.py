...
class ABM_Agent(mesa.Agent):
    ...
    def nominal_utility(self,customer_ID,date,nominal_utility_values):
        self.total_nominal_utility=(self.x1*self.Number_of_calls_O)+(self.x2*self.Data_usage*(1))+(self.x3*self.Complaint_calls)+(self.x4*self.SMS_sent)+(self.x5*self.SMS_received)
        customer_ID.append(self.CustomerID)
        date.append(self.month)
...
list1=list(zip(customer_ID,date,utility_values,nominal_utility_values))
df=pd.DataFrame(list1,columns=['CustomerId','Month','Utility','nominal_utility'])
...
df1["Churn_Propensity"]=(df1['Utility']-min_utility)/(max_utility-min_utility)
