...
churn_df = pd.read_csv(churn_data_file, names=['date', 'churn_rate', 'churn_bucket','cust_sentiment'], skiprows=1,  delimiter=',', dayfirst=True, parse_dates=True)
...
consum_df = pd.read_csv(syn_data_file, names=['customerid', 'date', 'numberofcalls_o', 'duration_o_min', 'numberofcalls_i', 'duration_i_min', 'datausage', 'complaintcalls', 'sms_sent', 'sms_received', 'services'], skiprows=1, delimiter=',', dayfirst=True, parse_dates=True)
...
def analytics_behavior():
    if request.method == 'GET':
        ...
        if from_date and to_date:
            ...
            churn_df_s = churn_df.loc[(pd.to_datetime(consum_df['date']).dt.date >= from_date) & (pd.to_datetime(consum_df['date']).dt.date <= to_date)]

            fin_df_s = fin_df.loc[(fin_df['month'] >= from_date.month) & (fin_df['month'] <= to_date.month)]

            consum_df_s = consum_df.loc[(pd.to_datetime(consum_df['date']).dt.date >= from_date) & (pd.to_datetime(consum_df['date']).dt.date <= to_date)]
            consum_df_cust_connect = consum_df_s[['date', 'complaintcalls']].groupby('date').sum().reset_index()
            consum_df_cust_connect["date"] = pd.to_datetime(consum_df_cust_connect["date"]).dt.strftime('%d-%m-%Y')
            ...
            response = {'overview_vals': {'customer_base': churn_df_s.date.nunique(),
                        ...
                        'cust_connect_vals': consum_df_cust_connect.to_dict(orient='records')}
        else:
            LOG.info('Analytics behaviour GET method is triggered for full dataset!')
            consum_df_cust_connect = consum_df[['date', 'complaintcalls']].groupby('date').sum().reset_index()
            consum_df_cust_connect["date"] = pd.to_datetime(consum_df_cust_connect["date"]).dt.strftime('%d-%m-%Y')
